...
router.post('/', requireAdmin, (req, res) => {
  const { name, description = '', image, images = [], port_mappings = [], env_vars = [], memory_limit = 512, cpu_limit = 1.0, disk_limit = 0, startup_command = '', install_script = '', pre_start_script = '', required_rank_id = null } = req.body;
  ...
  const id = uuidv4();
  db.prepare('INSERT INTO presets (id, name, description, image, images, port_mappings, env_vars, memory_limit, cpu_limit, disk_limit, startup_command, install_script, pre_start_script, required_rank_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
    .run(id, name, description, image, JSON.stringify(safeImages), JSON.stringify(port_mappings), JSON.stringify(env_vars), memory_limit, cpu_limit, Math.max(0, parseInt(disk_limit) || 0), startup_command, install_script, pre_start_script, required_rank_id || null);
...
router.put('/:id', requireAdmin, (req, res) => {
  ...
  const { name, description, image, images, port_mappings, env_vars, memory_limit, cpu_limit, disk_limit, startup_command, install_script, pre_start_script, required_rank_id } = req.body;
  ...
  db.prepare('UPDATE presets SET name=?, description=?, image=?, images=?, port_mappings=?, env_vars=?, memory_limit=?, cpu_limit=?, disk_limit=?, startup_command=?, install_script=?, pre_start_script=?, required_rank_id=? WHERE id=?')
    .run(
      name ?? existing.name,

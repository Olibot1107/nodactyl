...
const VALID_SCHEDULE_ACTIONS = new Set(['start', 'stop', 'restart', 'kill']);

router.get('/:id/schedules', (req, res) => {
  const server = db.prepare('SELECT * FROM servers WHERE id = ?').get(req.params.id);
  if (!server) return res.status(404).json({ error: 'Not found' });
  if (!canAccess(server, req.user)) return res.status(403).json({ error: 'Forbidden' });
  const rows = db.prepare('SELECT * FROM server_schedules WHERE server_id = ? ORDER BY hour, minute').all(req.params.id);
...
router.post('/:id/schedules', (req, res) => {
  ...
  if (!server) return res.status(404).json({ error: 'Not found' });
  if (req.user.role !== 'admin' && server.owner_id !== req.user.id) return res.status(403).json({ error: 'Forbidden' });
  const { action, hour, minute, days = [] } = req.body;
  if (!VALID_SCHEDULE_ACTIONS.has(action)) return res.status(400).json({ error: 'Invalid action' });
  const h = parseInt(hour), m = parseInt(minute);
  if (isNaN(h) || h < 0 || h > 23) return res.status(400).json({ error: 'Invalid hour (0–23)' });
  if (isNaN(m) || m < 0 || m > 59) return res.status(400).json({ error: 'Invalid minute (0–59)' });
  if (!Array.isArray(days) || days.some(d => ![0, 1, 2, 3, 4, 5, 6].includes(Number(d)))) return res.status(400).json({ error: 'Invalid days' });
  const count = db.prepare('SELECT COUNT(*) as n FROM server_schedules WHERE server_id = ?').get(req.params.id)?.n ?? 0;
  if (count >= 10) return res.status(400).json({ error: 'Maximum 10 schedules per server' });
  const id = uuidv4();
  db.prepare('INSERT INTO server_schedules (id, server_id, action, hour, minute, days) VALUES (?, ?, ?, ?, ?, ?)')
    .run(id, req.params.id, action, h, m, JSON.stringify(days.map(Number)));
  res.status(201).json({ id, server_id: req.params.id, action, hour: h, minute: m, days: days.map(Number), enabled: 1, last_run: null });
});
...
router.patch('/:id/schedules/:sid', (req, res) => {
  ...
  if (!server) return res.status(404).json({ error: 'Not found' });
  if (req.user.role !== 'admin' && server.owner_id !== req.user.id) return res.status(403).json({ error: 'Forbidden' });
  const schedule = db.prepare('SELECT * FROM server_schedules WHERE id = ? AND server_id = ?').get(req.params.sid, req.params.id);
  if (!schedule) return res.status(404).json({ error: 'Not found' });
  const updates = [], values = [];
  if (req.body.enabled !== undefined) { updates.push('enabled = ?'); values.push(req.body.enabled ? 1 : 0); }
  if (!updates.length) return res.status(400).json({ error: 'Nothing to update' });
  db.prepare(`UPDATE server_schedules SET ${updates.join(', ')} WHERE id = ?`).run(...values, req.params.sid);
  const updated = db.prepare('SELECT * FROM server_schedules WHERE id = ?').get(req.params.sid);
...
router.delete('/:id/schedules/:sid', (req, res) => {
  ...
  if (!server) return res.status(404).json({ error: 'Not found' });
  if (req.user.role !== 'admin' && server.owner_id !== req.user.id) return res.status(403).json({ error: 'Forbidden' });
  const { changes } = db.prepare('DELETE FROM server_schedules WHERE id = ? AND server_id = ?').run(req.params.sid, req.params.id);
